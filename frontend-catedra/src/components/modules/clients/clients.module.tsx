  ...
  const toggleUserStatus = async (id: string) => {
    try {
      ...
      setClients(
        clients.map((client) =>
          client.id === id
            ? {
                ...client,
                is_active: response.data?.is_active ?? client.is_active,
              }
            : client
        )
      );
      ...
      if (err instanceof Error) {
        setErrorToggle(err.message);
      }
      setErrorToggle("Error al cambiar el estado del cliente");
    }
  };
  ...
  return (
    <div className="bg-white rounded-lg shadow-md p-6 mt-8">
      ...
      <div className="overflow-x-auto">
        {errorToggle && (
          <div className="text-center text-red-500">{errorToggle}</div>
        )}
            ...
            {clients.length === 0 ? (
              ...
              clients.map((client) => (
                <tr key={client.id}>
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                    $
                    {typeof client.monthly_income === "number"
                      ? client.monthly_income.toFixed(2)
                      : "0.00"}
                  </td>
...
export const ClientsModule = () => {
      ...
      try {
        ...
        if (response.success && response.data) {
          const processedClients = response.data.map((client) => ({
            ...client,
            phones: Array.isArray(client.phones)
              ? client.phones
              : client.phones
              ? [client.phones]
              : [],
            emails: Array.isArray(client.emails)
              ? client.emails
              : client.emails
              ? [client.emails]
              : [],
  ...
  const handleInputChange = (e: InputChangeEvent) => {
    ...
    setClientData((prevData) => ({
      ...
      [name]:
        name === "age" || name === "monthly_income" ? Number(value) : value,
    }));
  };
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (isNaN(parsedAge) || parsedAge <= 0) {
      setErrorForm("Age must be a positive number.");
      return;
    }
    if (isNaN(parsedMonthlyIncome) || parsedMonthlyIncome < 0) {
      setErrorForm("Monthly income must be a non-negative number.");
      return;
    }
    if (clientData.password.length < 6) {
      // Example validation for password length
      setErrorForm("Password must be at least 6 characters long.");
      return;
    }
    ...
    try {
      const dataToSend: CreateClientInput = {
        ...
        marital_status: clientData.marital_status as
          | "single"
          | "married"
          | "divorced"
          | "widowed",
      ...
      if (response.success && response.data) {
        const newClient: Client = {
          ...
          phones: Array.isArray(response.data.phones)
      ...
      if (err instanceof Error) {
        setErrorForm(err.message || "Error creating client");
      } else {
        setErrorForm("Error creating client");
      }
    }
  };
    ...
    <div className="p-6 bg-gray-100 min-h-screen">
      <div className="bg-white rounded-lg shadow-md p-6 mb-8">
        ...
        <form onSubmit={handleSubmit}>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-6">
            ...
            <div>
              <label
                ...
                className="block text-sm font-medium text-gray-700"
              >
                Correo Electrónico (separar con comas)
              </label>
              <input
                type="text"
                id="emails"
          ...
          {errorForm && (
            <p className="text-red-500 text-sm mt-4">{errorForm}</p>
          )}
